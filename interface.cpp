...
public:
  ...
  Eigen::VectorXd act(Eigen::VectorXd state) override {
    ...
    if (this->_current_iteration < Params::init_randomsampling::samples()) {
      Eigen::VectorXd starting_point = tools::random_vector(this->_model.dim_in(), Params::bayes_opt_bobase::bounded());
  ...
  void update(Eigen::VectorXd sample, Eigen::VectorXd observation) override {
    ...
    else if(this->_current_iteration > Params::init_randomsampling::samples()){
  ...
  Eigen::VectorXd best_arm_prediction(Eigen::VectorXd state) override {
    ...
    Eigen::VectorXd best_arm =
        this->_samples[best_arm_index];

    //TODO: best_arm_prediction should also return reward prediction and uncertainty (should be returned as vector of size dim + 2?)
    Eigen::VectorXd reward_prediction = this->_model_prediction(best_arm,state);
    Eigen::VectorXd result(EvalHandler::dim_in() + 2);
    result << best_arm, reward_prediction;
    return result;
  }

  Eigen::VectorXd _model_prediction(Eigen::VectorXd params,Eigen::VectorXd state) {
    Eigen::VectorXd reward_prediction = this->_model.mu(params);
    double uncertainty = this->_model.sigma(params) - this->_model.kernel_function().noise();
...
protected:
  Eigen::VectorXd get_state() override {
    //TODO : split from the optimizer interface itself?
